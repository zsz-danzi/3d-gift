...
Gift.prototype.init = function(opt){
	...
	this.ribbon_flag = opt.ribbon_flag == false ? false : true;  //礼带食是否显示
	...
	this.loadModel(function(object){
		...
		//设置纹理
		self.setTexture(self.img_url_arr,function(){
...
//设置纹理
Gift.prototype.setTexture = function(url_arr,callback){
...
//礼带显示
Gift.prototype.ribbon_show = function(){
	this.ribbon_flag = true;
	this.ribbonFn();
}

//礼带隐藏
Gift.prototype.ribbon_hide = function(){
	this.ribbon_flag = false;
	this.ribbonFn();
}

//礼带逻辑处理
Gift.prototype.ribbonFn = function(){
	if(!this.object) return;
...
Gift.prototype.animate = function(){
	...
	if(this.finger_move){
        ...
        //缓冲运动 速度 = （目标 - 当前）/ 摩擦系数
        this.object.rotation.y += (this.theta - this.object.rotation.y) * 0.01 ;  
